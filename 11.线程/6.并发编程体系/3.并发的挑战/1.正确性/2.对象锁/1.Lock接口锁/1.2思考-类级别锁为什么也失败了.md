# 按理说，类级别的应该会成功，因为是针对一个类，为什么失败了？=>没有失败，是自己的包冲突了，所以是可以的
从这个转账例子中看本质！+>关键在于需要维护同一数据类型属性的锁，方可起作用！！

## 所以从根本来看，没有对象锁，只有维护同一属性数据的锁，可以是对象，可以是类，还可以是代码块(转化为对象)
![alt text](../../../../../img/转账的类级别实现，为什么也失败了.png)
